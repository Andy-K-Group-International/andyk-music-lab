...
export async function POST(req: NextRequest) {
  ...
  const sbRes = await fetch(`${SUPABASE_URL}/rest/v1/lab_waitlist`, {
    method: "POST",
    headers: { ...sbHeaders(), "Prefer": "return=minimal" },
    body: JSON.stringify({ email: emailClean, name: nameClean, plan: plan || "studio" }),
  });

  if (sbRes.status === 409) return NextResponse.json({ ok: false, duplicate: true }, { status: 409 });
  ...
  if (apiKey) {
    ...
    await Promise.allSettled([
      sendEmail(apiKey, {
        from: FROM,
        to: emailClean,
        subject: "You're on the list — Andy'K Music Lab",
        html: confirmationHtml(emailClean, nameClean, planLabel),
      }),
      sendEmail(apiKey, {
        from: FROM,
        to: "ceo@andykgroup.com",
        subject: `New waitlist signup — ${plan || "studio"}`,
        html: adminHtml(emailClean, nameClean, planLabel, timestamp),
      }),
    ]);
  }
...
export async function GET() {
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/lab_waitlist?select=*&order=created_at.desc`,
