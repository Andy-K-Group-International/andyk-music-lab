...
// ── POST /api/waitlist/[id] — send launch email to one person ─────────────────
...
export async function POST(
  ...
  const entryRes = await fetch(
    `${SUPABASE_URL}/rest/v1/lab_waitlist?id=eq.${id}&select=*&limit=1`,
    { headers: sbHeaders() }
  );
  ...
  // Mark notified
  await fetch(`${SUPABASE_URL}/rest/v1/lab_waitlist?id=eq.${id}`, {
    method: "PATCH",
